...
## Project-Specific Rules — mvp-locaweb

> Adaptação local (não faz parte do conteúdo original da skill). Estas regras têm precedência sobre a orientação genérica acima sempre que houver conflito, para qualquer forecast produzido neste projeto.

- Usar **backtest rolling-origin** (ver `references/prophet-cv.md`) — nunca validar com um único split treino/teste.
- Avaliar separadamente os horizontes **D+1 até D+7**, não só o horizonte agregado.
- Comparar sempre contra um **baseline sazonal ingênuo** (ex.: valor do mesmo dia da semana anterior) — só reportar o modelo Prophet como útil se ele bater esse baseline.
- Calcular **MAE, RMSE e WAPE** a cada avaliação.
- Avaliar a **cobertura dos intervalos de previsão** (`yhat_lower`/`yhat_upper`) e reportar se está abaixo do nominal (ver seção "Coverage Diagnostic" em `references/prophet-cv.md`).
- Executar a avaliação nos seguintes cortes, quando os dados permitirem:
  - total;
  - prioridade;
  - categoria;
  - equipe (quando disponível).
- Verificar **autocorrelação dos resíduos** — lembrar da IRON LAW acima: Prophet não modela autocorrelação, então resíduos autocorrelacionados indicam intervalos de incerteza subestimados.
- **Documentar** os valores usados de `changepoint_prior_scale`, `seasonality_prior_scale` e `seasonality_mode` em toda execução/relatório de forecast.
- **Não escolher hiperparâmetros usando o conjunto de teste final** — tuning é feito via cross-validation (ver `references/prophet-tuning.md`), o teste final fica intocado até a avaliação de aceitação.
- **Não usar MAPE isoladamente** quando houver valores próximos de zero na série (MAPE explode nesse caso) — preferir WAPE ou MAE nesses cortes.
